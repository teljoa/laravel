...
class SavetasksRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            'user' => 'required|string|min:3|max:20|unique:tasks,user,' . ($this->route('task') ? $this->route('task')->id : ''),
            'name' => 'required|string|max:50',
            'tasks' => 'required|string|max:50',
            'email' => 'required|email|max:100|unique:tasks,email,' . ($this->route('task') ? $this->route('task')->id : '')
        ];
